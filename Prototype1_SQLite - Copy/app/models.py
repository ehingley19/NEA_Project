...
class Teacher(db.Model):
    ...
    class_of_students: so.WriteOnlyMapped['Student'] = so.relationship(back_populates='teacher')

    def __repr__(self):
        return f'<Type: teacher> <Name: {self.forename} {self.surname}> <Username: {self.username}> \
        <Email: {self.email}>'
...
class Student(db.Model):
    ...
    class_teacher: so.Mapped['Teacher'] = so.relationship(back_populates='students')

    def __repr__(self):
        return f'<Type: student> <Name: {self.forename} {self.surname}> <Username: {self.username}> \
        <Email: {self.email}>'
